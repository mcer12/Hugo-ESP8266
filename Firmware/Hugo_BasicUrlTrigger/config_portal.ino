...
void handleRoot() {
  ...
  html += "\"></div>";
  html += "<h2>Buttons settings</h2><p>Assign target URL for each button.<br>You can also use [blvl] shortcode to add battery percentage and [mac] for mac address (as unique identifier).<br>For example: \"http://example.com/?trigger=button1&battery_percentage=[blvl]&mac=[mac]\"</p>";
